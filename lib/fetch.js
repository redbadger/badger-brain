...
function makeGetter(json) {
  return key => R.pathOr(null, ['data', key, 'value'], json);
}
...
export function mapLinkList(linkList) {
  ...
  return linkList.map((link) => {
    ...
    const linkItem = {};
    linkItem.title = R.pathOr('', ['label', 'value'], link);
    linkItem.url = R.pathOr('', ['link', 'value'], link);
    return linkItem;
...
export function sanitizeEventAndNews(item, type) {
  ...
  return {
    ...
    datetime: mapDate(get(`${type}.timestamp`)),
