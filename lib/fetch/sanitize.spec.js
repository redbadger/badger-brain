...
describe('data/sanitize', () => {
  describe('sanitizeEvent', () => {
    ...
    it('should extract eventbrite event id from eventbrite events', () => {
      const event = deepFreeze({
        id: '123',
        uid: 'event-title',
        tags: ['tag1', 'tag2'],
        data: {
          ...
          'event.ticketLink': {
            value:
              'https://www.eventbrite.com/e/elrow-town-london-2019-tickets-49690716270',
          },
          'event.streamingLink': { value: 'streamingLinkExample' },
          'event.status': { value: 'WAITLIST' },
          'event.talks': { value: ['event', 'talks'] },
      ...
      const eventbriteId = '49690716270';

      expect(sanitizeEvent(event).eventbriteId).to.deep.equal(eventbriteId);
    });
  ...
  describe('sanitizeBadger', () => {
    ...
    it('converts the raw json provied by prismic to be readable by graphql', () => {
      ...
      expect(result).to.deep.equal({
        ...
        hasBlogPosts: true,
        categories: [
          { name: 'Engineering', order: 6, slug: 'engineering' },
    ...
    it('converts valid tags into categories', () => {
      ...
      expect(result).to.deep.equal({
        ...
        categories: [
          { name: 'Leadership', order: 1, slug: 'leadership' },
          { name: 'advisory board', order: 2, slug: 'advisory-board' },
          { name: 'Strategy', order: 3, slug: 'strategy' },
          { name: 'Delivery', order: 4, slug: 'delivery' },
          { name: 'UX & Design', order: 5, slug: 'ux-design' },
          { name: 'Engineering', order: 6, slug: 'engineering' },
          { name: 'QA', order: 7, slug: 'qa' },
          { name: 'Operations', order: 8, slug: 'operations' },
          { name: 'Marketing', order: 9, slug: 'marketing' },
          { name: 'Insights', order: 10, slug: 'insights' },
          { name: 'Adorable', order: 11, slug: 'adorable' },
        ],
      });
    });
