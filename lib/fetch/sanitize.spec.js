import * as prismicEventResponse
  from '../../test/fixtures/event-prismic-response.json';
...
describe.only('data/sanitize', () => {
  describe('sanitizeEvent', () => {
    ...
    it(
      'should append URL of the featured image if no full URL is available',
      () => {
        const event = deepFreeze({
          id: '123',
          data: {
    ...
    it(
      'should copy start datetime to end datetime when end datetime is empty',
      () => {
  ...
  describe('sanitizeSpeaker', () => {
    it(
      'converts the raw json provied by prismic to be readable by graphql',
      () => {
        const rawData = {
          id: 'exampleId',
          data: {
  ...
  describe('sanitizeOrganisation', () => {
    ...
    it(
      'returns the default values if the fields are not passed from prismisc',
      () => {
        const rawData = {
          id: null,
          data: {},
