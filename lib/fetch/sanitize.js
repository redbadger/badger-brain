...
function sanitizeEventAndNews(item, type) {
  ...
  const featureImageFilename = pathOr(null, ['main', 'url'], get(`${type}.${imageField}`));
...
export function sanitizeOrganisation(json) {
  const get = makeGetter(json);
  const organisationImage = pathOr(null, ['main', 'url'], get('organisation.organisation-image'));
  ...
  return {
    ...
    imageURL: organisationImage || fallbackOrganisationImage,
...
export function sanitizeSpeaker(json) {
  const get = makeGetter(json);
  const profileImageURL = pathOr(null, ['main', 'url'], get('speaker.profile-image'));
...
export function sanitizeBadger(json) {
  ...
  const primaryImageUrl = pathOr(null, ['main', 'url'], get('badger.primary-image'));
  const secondaryImageUrl = pathOr(null, ['main', 'url'], get('badger.secondary-image'));
  ...
  return {
    ...
    primaryImageUrl: primaryImageUrl || sanitizeUrl(fallbackPrimaryImageUrl),
    secondaryImageUrl: secondaryImageUrl || sanitizeUrl(fallbackSecondaryImageUrl),
