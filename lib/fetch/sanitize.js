...
export function sanitizeBadger(json) {
  ...
  return {
    ...
    primaryImageUrl: primaryImageUrl &&
      primaryImageUrl.replace(/^http:/, 'https:'),
    secondaryImageUrl: secondaryImageUrl &&
      secondaryImageUrl.replace(/^http:/, 'https:'),
...
export function sanitizeQnA(json) {
  const category = {
    name: pathOr(
      null,
      ['data', 'q-and-a-category.category-name', 'value'],
      json,
    ),
    slug: json.slug,
    topics: pathOr(
      null,
      ['data', 'q-and-a-category.q-n-a-list', 'value'],
      json,
    ),
  };
  return category;
...
export function sanitizeQnATopic(json) {
  const answerFragment = pathOr(null, ['data', 'q-and-a.answer'], json);
  const answerHtml = answerFragment
    ? Prismic.Fragments.initField(answerFragment).asHtml()
    : null;

  return {
    slug: json.slug,
    question: pathOr(null, ['data', 'q-and-a.question', 'value'], json),
    answer: answerHtml,
  };
}
