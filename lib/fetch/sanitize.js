...
export function sanitizeQnA(json) {
  const category = {
    name: json.data['q-and-a-category.category-name'].value,
    slug: json.slug,
    topics: json.data['q-and-a-category.q-n-a-list'].value,
  };
  return category;
}
...
export function sanitizeQnATopic(json) {
  return {
    question: json.data['q-and-a.question'].value,
    answer: Prismic.Fragments.initField(json.data['q-and-a.answer']).asHtml(),
  };
}
