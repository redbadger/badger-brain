...
function mapConsultants(consultants) {
  return consultants
    .filter((consultant) => consultant)
    .map((consultant) => consultant.document.slug);
}
export async function sanitizeGoldCoinPage(json, options) {
  const get = makeGetter(json);
  const headerImage = {
    main: get('gold_coin_page.header_image').main.url,
    large: get('gold_coin_page.header_image').views.large.url,
    medium: get('gold_coin_page.header_image').views.medium.url,
    small: get('gold_coin_page.header_image').views.small.url,
  };
  const headerAlt = get('gold_coin_page.header_image').main.alt;
  const consultants = mapConsultants([
    get('gold_coin_page.consultant_1'),
    get('gold_coin_page.consultant_2'),
    get('gold_coin_page.consultant_3'),
  ]);
  const hubspotForm = await options.formFetcher({
    id: get('gold_coin_page.hubspot_form_id'),
  });
  return {
    // explicitly assigning false here because prismic
    // doesn't sent false boolean values through their api
    unlisted: get('gold_coin_page.unlisted') || false,
    slug: json.slug,
    title: get('gold_coin_page.title'),
    subTitle: get('gold_coin_page.subtitle'),
    headerImage,
    headerAlt,
    duration: get('gold_coin_page.duration'),
    price: get('gold_coin_page.price'),
    type: get('gold_coin_page.type'),
    location: get('gold_coin_page.location'),
    whatIsIt: sanitizeHtml(
      new Prismic.Fragments.StructuredText(
        get('gold_coin_page.what_is_it')
      ).asHtml()
    ),
    whoIsItFor: sanitizeHtml(
      new Prismic.Fragments.StructuredText(
        get('gold_coin_page.who_is_it_for')
      ).asHtml()
    ),
    whatWillYouLearn: sanitizeHtml(
      new Prismic.Fragments.StructuredText(
        get('gold_coin_page.what_will_you_learn')
      ).asHtml()
    ),
    whoWillRun: sanitizeHtml(
      new Prismic.Fragments.StructuredText(
        get('gold_coin_page.who_will_run')
      ).asHtml()
    ),
    consultants,
    hubspotForm,
...
function sanitizeHubspotFormFields(fields) {
  return fields.map((field) => {
    const richTextContent = field.richText.content
      ? sanitizeHtml(field.richText.content)
      : null;
    let result;
...
function sanitizeHubspotConsentData(json) {
  const rawData = JSON.parse(
    json.find((data) => data.name === 'legalConsentOptions').value
  );
  if (rawData) {
    return {
      consentMessage: sanitizeHtml(rawData.privacyPolicyText),
...
export function sanitizeHubspotForm(json) {
  if (json) {
    return {
      ...
      formConsent: sanitizeHubspotConsentData(json.metaData),
    };
  }
  return null;
}

export function sanitizeJobs(json) {
  if (json) {
    return json.map((job) => ({
      id: job.id,
      title: job.title,
      description: job.content ? job.content.substring(0, 360) : '',
      fullDescription: job.content,
      department: job.departments[0] ? job.departments[0].name : '',
      applicationUrl: job.absolute_url,
      slug: job.title ? job.title.toLowerCase().replace(/ /gi, '-') : '',
...
export function sanitizeBlogPostExcerpts(post) {
  return {
