...
describe('Fetch', () => {
  ...
  describe('All', () => {
    it('should fetch events', async () => {
      const scope = prismicMock.mockDocumentTypeQuery('event');
    ...
    it('should fetch news', async () => {
      const scope = prismicMock.mockDocumentTypeQuery('news');
    ...
    it('should fetch tagged events', async () => {
      const tag = 'falafel';
      const scope = prismicMock.mockDocumentTypeTagQuery('event', tag);

      await fetchAllEvents(undefined, { tag });

      expect(scope.isDone()).to.equal(true);
    });

    it('should fetch tagged news', async () => {
      const tag = 'falafel';
      const scope = prismicMock.mockDocumentTypeTagQuery('news', tag);

      await fetchAllNews(undefined, { tag });
