...
export function sanitizeWebinar(json) {
  ...
  return {
    ...
    endDateTime: pathOr(get('webinar.timestamp'),
      ['data', 'webinar.timestampEnd', 'value'], json),
    featureImageFilename: pathOr(
      null,
      ['main', 'url'], get('webinar.featured-image')),
...
export function sanitizeEventsBanner(json) {
  const get = makeGetter(json);
  return {
    url: pathOr(null, ['url'], get('events_list_banner.banner_link')),
    desktopImageURL: pathOr(
      null,
      ['main', 'url'], get('events_list_banner.banner_image_desktop')),
    tabletImageURL: pathOr(
      null,
      ['main', 'url'], get('events_list_banner.banner_image_tablet')),
    mobileImageURL: pathOr(
      null,
      ['main', 'url'], get('events_list_banner.banner_image_mobile')),
  };
}
