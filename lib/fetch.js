import Prismic from 'prismic.io';
import dateFns from 'date-fns';
import { PrismicConfig } from './config/prismic.js';

export function mapDate(timestamp) {
  const d = new Date(timestamp);
  return {
    iso: timestamp,
    date: dateFns.format(d, 'DD'),
    month: dateFns.format(d, 'MM'),
    monthSym: dateFns.format(d, 'MMM'),
    year: dateFns.format(d, 'YYYY'),
  };
}

export function mapLinkList(linkList) {
  if (linkList) {
    return linkList.map((link) => {
      const linkItem = {};
      if (!link.hasOwnProperty('label') || !link.hasOwnProperty('link')) {
        return undefined;
      }
      linkItem.title = (link.label && link.label.hasOwnProperty('value')) ?
        link.label.value : '';
      linkItem.url = (link.link && link.link.hasOwnProperty('value')) ?
        link.link.value : '';
      return linkItem;
    }).filter(l => l !== undefined);
  }

  return [];
}

export function sanitizeEventAndNews(item, type) {
  const imageField = (type === 'news') ? 'featureImage' : 'event-image';
  return {
    id: item.id,
    slug: item.slug ? item.slug : null,
    tags: item.tags || [],
    title: item.data[`${type}.title`] ?
      item.data[`${type}.title`].value : null,
    strapline: item.data[`${type}.strapline`] ?
      item.data[`${type}.strapline`].value : null,
    datetime: item.data[`${type}.timestamp`] ?
      mapDate(item.data[`${type}.timestamp`].value) : null,
    internalLinks: item.data[`${type}.internalLinks`] ?
      mapLinkList(item.data[`${type}.internalLinks`].value) : [],
    externalLinks: item.data[`${type}.externalLinks`] ?
      mapLinkList(item.data[`${type}.externalLinks`].value) : [],
    body: item.data[`${type}.body`] ?
      item.data[`${type}.body`].value : [],
    featureImageFilename: item.data[`${type}.${imageField}`] ?
      item.data[`${type}.${imageField}`].value : null,
  };
}

export function getIn(key, defaultValue, json) {
  const datum = json.data[key];
  return datum ? datum.value : defaultValue;
}

export function sanitizeCommunity(json) {
  return {
    id: json.id,
    communityTitle: getIn('community.communityTitle', null, json),
    communitySummary: getIn('community.communitySummary', null, json),
    mailingListTitle: getIn('community.mailingListTitle', null, json),
  };
}

export function sanitizeSpeaker(json) {
  return {
    id: json.id,
    name: getIn('speaker.name', null, json),
    company: getIn('speaker.company', null, json),
    talkTitle: getIn('speaker.talkTitle', null, json),
    talkSummary: getIn('speaker.talkSummary', null, json),
    twitterHandle: getIn('speaker.twitterHandle', null, json),
    githubHandle: getIn('speaker.githubHandle', null, json),
    blogURL: getIn('speaker.blogURL', null, json),
    imageURL: getIn('speaker.imageURL', null, json),
  };
}

async function fetchOne(_, args, req, docType, sanitizeFunction) {
  const ref = req.headers['x-preview'];

  const api = await Prismic.api(PrismicConfig.apiEndpoint);
  const res = await api.form('everything').ref(ref || api.master()).query(
    Prismic.Predicates.at('document.id', args.id)
  )
  .submit();

  if (res && res.results.length > 0) {
    return sanitizeFunction(res.results[0], docType);
  }

  return {};
}

async function fetchAll(docType, sanitizeFunction) {
  const api = await Prismic.api(PrismicConfig.apiEndpoint);
  const res = await api.form('everything').ref(api.master()).query(
    Prismic.Predicates.at('document.type', docType)
  )
  .pageSize(100)
  .submit();

  if (res && res.results.length > 0) {
    return res.results.map((item) => sanitizeFunction(item, docType));
  }
  return [];
}


export async function fetchEventsByCommunityId(communityId) {
  try {
    const api = await Prismic.api(PrismicConfig.apiEndpoint);
    const res = await api.form('everything').ref(api.master()).query(
      Prismic.Predicates.at('document.type', 'event'),
      Prismic.Predicates.has('my.event.community', communityId)
    )
    .submit();
    if (res && res.results.length > 0) {
      return res.results.map((item) => sanitizeEventAndNews(item, 'event'));
    }
  } catch (e) {
    return [];
  }
  return [];
}

export async function getSpeakersByEventId(eventId) {
  const api = await Prismic.api(PrismicConfig.apiEndpoint);
  try {
    const res = await api.form('everything').ref(api.master()).query(
      Prismic.Predicates.at('document.type', 'speaker'),
      Prismic.Predicates.has('my.speaker.event', eventId)
    )
    .submit();
    if (res && res.results.length > 0) {
      return res.results.map(sanitizeSpeaker);
    }
  } catch (e) {
    return [];
  }
  return [];
}

/* Community */
export async function fetchAllCommunities() {
  return fetchAll('community', sanitizeCommunity);
}

export async function fetchCommunity(_, args, req) {
  return fetchOne(_, args, req, 'event', sanitizeCommunity);
}

/* Speaker */
export async function fetchAllSpeakers() {
  return fetchAll('speaker', sanitizeSpeaker);
}

export async function fetchSpeaker(_, args, req) {
  return fetchOne(_, args, req, 'speaker', sanitizeSpeaker);
}

/* Event */
export async function fetchAllEvents() {
  return fetchAll('event', sanitizeEventAndNews);
}

export async function fetchEvent(_, args, req) {
  return fetchOne(_, args, req, 'event', sanitizeEventAndNews);
}

/* News */
export async function fetchAllNews() {
  return fetchAll('news', sanitizeEventAndNews);
}

export async function fetchNewsArticle(_, args, req) {
  return fetchOne(_, args, req, 'news', sanitizeEventAndNews);
}
