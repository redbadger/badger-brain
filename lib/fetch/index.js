...
import {
  ...
  sanitizeBlogPosts,
  sanitizeBlogPostExcerpts,
  sanitizeHubspotForm,
  sanitizeGoldCoinPage,
  sanitizeJobs,
} from './sanitize';

export async function fetchHubspotForm(args) {
...
export async function fetchHubspotFormsById(args) {
  return args.ids.map((id) => {
    const result = fetchHubspotForm({ id });
    return result;
  });
}
...
async function fetchAll(docType, sanitizeFunction, args) {
  ...
  if (docType === 'gold_coin_page') {
    options.formFetcher = fetchHubspotForm;
  }
...
// GoldCoinPages
export async function fetchAllGoldCoinPages() {
  // Prismic doesn't allow for required fields
  // And graphql doesn't elegantly remove null values
  // from a list with failed records.
  // This is a workaround.
  const pages = await fetchAll('gold_coin_page', sanitizeGoldCoinPage);

  const FAIL_TOKEN = Symbol('failed');

  const resolvedPages = Promise.all(
    pages.map((p) => p.catch(() => FAIL_TOKEN))
...
export function fetchBlogPostExcerptsByTag(args) {
  return fetchAllHubspot(sanitizeBlogPostExcerpts, args);
}

export async function fetchAllJobs() {
  const response = await fetch(
    'https://boards-api.greenhouse.io/v1/boards/redbadger/jobs?content=true'
  );
  const results = await response.json();

  return sanitizeJobs(results.jobs);
}
